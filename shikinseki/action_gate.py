...
def check(actions, state, invariants):
...
def run(policy, state, invariants, verbose=True):
...
def inv_required_fields(fields):
...
def inv_not_contradict(key, conflicting_pairs):
...
def inv_expected_count(expected_fn, severity="error"):
