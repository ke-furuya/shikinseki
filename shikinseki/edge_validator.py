...
# ── 1. ROI と信頼区間 ──
def roi_ci(records, iters=2000, seed=42):
    """acted行（cost>0）の ROI（払戻/賭け金×100）と group単位90%CI。
    CI下限>100なら『頑健にプラス』。点推定だけ見ると分散に騙される。"""
...
def mean_ci(records, value_key="value", iters=2000, seed=42):
    ...
    return {"mean": point, "ci_low": lo, "ci_high": hi, "n": len(rs),
            "n_groups": len(groups), "reliable": len(groups) >= MIN_GROUPS,
            "robust_positive": (lo is not None and lo > 0),
            "robust_negative": (hi is not None and hi < 0)}


# ── 1d. 信頼区間列（逐次的に母数を増やしても妥当・覗き見OK）──
def confidence_sequence(values, alpha=0.10, sigma=None, rho=0.5):
...
def power_required(records, target_roi=110.0, measurable_n=5000):
    ...
    return {"sd_per_100": sd, "se_at_n": se_per_n(len(acted)),
            "n_now": len(acted), "need_n_for_target": need,
            "measurable": need <= measurable_n, "measurable_n": measurable_n,
            "skewness": sk, "heavy_tailed": abs(sk) > 2}


def power_required_mean(values, mde, measurable_n=5000, two_sample=False):
...
def residual_scan(records, feature_specs, nbins=15, value_key="outcome", seed=42):
...
def holdout_scan(records, feature_specs, split_time=None, nbins=15, value_key="outcome", seed=42):
...
def multiple_comparison_note(n_tests, z=2.5):
...
def pbo_cscv(perf_matrix, n_blocks=10):
...
def purged_kfold_indices(times, n_splits=5, embargo_frac=0.01):
...
def staged_scan(records, feature_specs, estimand="", nbins=15, value_key="outcome", seed=42):
...
def cohens_kappa(labels_a, labels_b):
    ...
    return {"kappa": kappa, "percent_agreement": po, "n": n, "n_categories": len(cats)}
...
def decision_value_ab(effect_mean, effect_lo, value_per_unit, switch_cost=0.0):
...
def value_of_information(effect_mean, se, value_per_unit):
...
def value_of_more_data(effect_mean, se, value_per_unit, sampling_cost):
...
# ── 整形出力ヘルパ ──
def print_report(title, **sections):
